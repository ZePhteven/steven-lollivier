...
formatContent = function (content) {
  ...
  for (var i = 0; i < content.length; i++) {
    ...
    } else output += '<span class="js-out">' + content[i] + "</span>";
  }
  return output;
  // return content;
};
...
animateOutput = function (item, i, length) {
  setTimeout(function () {
    ...
    if (i === length - 1) inprogress = false;
  }, queueTime * i);
};
...
showContent = function (content, command = false, flush = false) {
  ...
  q.queue(function () {
    var uniqueId = "";
    if (!command) uniqueId = Math.floor(Math.random() * 100000 + 1);
...
iterate = function (obj, items) {
  ...
  for (var key in obj) {
    ...
    } else items.push("<p>" + setSpacer(level) + key + " : " + elem + "</p>");
  }
};
...
getFunctionList = function (namespace, array) {
  $.get("js/" + namespace + ".js", function (data) {
    ...
    for (var key in x) {
      ...
      if (typeof array[section] === "undefined") array[section] = new Array();
...
getItems = function (items, message) {
  if (typeof items === "undefined") showContent(message);
  else {
