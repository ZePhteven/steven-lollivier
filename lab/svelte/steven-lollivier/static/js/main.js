...
$(function () {
	...
	$(window).smartload(function () {
		if ($.fn.matchHeight) $('.match-height').matchHeight();
	});
	...
	var pageScroll = function () {
		$('.page-scroll > a').bind('click', function (e) {
			 ...
			 */
			if (!$(this).parent().hasClass('dropdown')) $('.main-collapse').collapse('hide');
		});
	};
	pageScroll();
	...
	var toggleNavbarMethod = function () {
		...
		} else dropdown.off('mouseover').off('mouseout');
	};
	toggleNavbarMethod();
	...
	var circleChart = function () {
		$('.circle-chart')
			...
			.each(function () {
				...
				} else item.css('height', newHeight);
			});
	};
	...
	var counter = function () {
		...
		if ($.fn.countTo) number.countTo({ speed: 3000 });
	};
